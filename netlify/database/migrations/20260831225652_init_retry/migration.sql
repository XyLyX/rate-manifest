-- Rate Manifest — initial schema for Netlify DB (Postgres). A retry of
-- 20260831220000_init, which never actually ran: its INSERTs called
-- gen_random_uuid(), that failed on Netlify's Postgres, and because the
-- whole file runs as one transaction, the failure rolled back every
-- CREATE TABLE above it too — every table came back missing on every
-- deploy after. Netlify tracks that migration name as already-applied
-- regardless, so fixing 20260831220000_init's content in place wouldn't
-- get it re-run — hence a new migration folder with the corrected SQL.
-- See DECISIONS.md, "Bug: the migration never actually ran," for the full
-- story. That original file is left in place, untouched, as a harmless
-- no-op — do not delete it; Netlify's applied-migrations record still
-- references it.
--
-- Mirrors src/db/schema.ts exactly (see that file for the field-by-field
-- rationale). Netlify applies migrations automatically, in order, before
-- every production deploy and before each deploy preview's first deploy on
-- a new branch — see DECISIONS.md, "Hosting: Netlify DB".
--
