...
const EMPTY_STATE_CARDS: PillarCard[] = [
  {
    ...
    title: "What a hotel actually costs",
    body: "The headline nightly rate in any rate comparison is rarely the final figure. Taxes, resort fees, service charges, and destination-specific levies are applied differently by property, country, and booking channel — meaning the lowest-looking rate from one source can cost more on checkout than a higher-looking rate from another. Rate Manifest compares total costs across sources, not just the headline figures, and shows you where the difference comes from.",
  ...
  },
  {
    ...
    title: "Decisions that affect the outcome",
    body: "Timing, district, and booking source are often more consequential than the choice of property. A hotel in the wrong neighbourhood for your purpose, booked during peak event demand through the wrong channel, can cost significantly more and deliver less than an apparently similar option selected with better information. Rate Manifest surfaces the context — timing signals, district positioning, source terms and cancellation conditions — alongside the rate so you can make a genuinely informed comparison.",
    freshnessState: "valid",
    isResearched: false,
  },
  {
    ...
    body: "How you reach a destination and move around it directly affects which accommodation makes sense. Airport proximity, transit connections, transfer costs, and local transport options all bear on whether a given property is well-positioned for your actual itinerary — not just whether it is central on a map. Rate Manifest shows property location in the context of the destination's key arrival points and transit infrastructure.",
    freshnessState: "valid",
    isResearched: false,
  },
  {
    ...
    body: "Active events that compress hotel supply, cultural norms that shape what to expect on arrival, current conditions that affect planning — these are part of the travel decision but rarely surface in a standard rate comparison. Rate Manifest's Local Pulse dimension surfaces the destination context that matters, drawn from verified sources, so the rate you're evaluating sits inside the full picture of the trip.",
    freshnessState: "valid",
    isResearched: false,
  },
];
