...
## Klook travel-essentials widget added (2026-09-03)

Closes a gap opened by "Klook Tours Widget removed - redundant with
native Viator (2026-09-03)" above: once that widget was gone,
KLOOK_EXPERIENCE_CATEGORIES ("Airport transfers", "Transport",
"SIM/eSIM") was just a sentence of copy with no real cards behind
it, sitting right below Viator's rich native grid - a visible step
down in the same block. The user flagged this directly ("the klook
airport transfer, transport, sim/esim widgets vanished"), though as
explained back to them at the time, there had never actually been
per-category widgets before either - only the one general Tours
Widget (showing generic tour/activity cards, not category-specific
ones) plus this same text sentence. What changed is that removing
the Tours Widget took away the only real widget in the block,
making the sentence-only version look thin by comparison for the
first time.

Fix: a new, real widget covering exactly those three categories.
Went back to Klook's own Affiliate Dashboard (My Ads -> Other
tools -> Dynamic Widgets), this time NOT scoped to Hotels. That
generator exposes category targeting as a "tid" (tag id) rather
than a named dropdown, so which tid maps to which content isn't
visible until generated - the user tried two:

- tid "5" (adid "1413967", amount 3): live preview showed "Dubai
  International Airport Lounge Service by Marhaba", "Dubai Private
  Car Charter", and "5G eSIM United Arab Emirates (UAE) | Etisalat"
  - a genuine match for all three KLOOK_EXPERIENCE_CATEGORIES in one
  widget. This is the one wired in - KLOOK_ESSENTIALS_WIDGET_CONFIG
  in klook.ts.
- tid "21" (adid "1413968", amount 6): live preview showed
  attraction tickets - Sky Views Observatory, At The Top (Burj
  Khalifa), The View At The Palm. Real Klook content, but
  attractions/tours - exactly the category Viator's native section
  already covers, and exactly the redundancy the Tours Widget
  removal was meant to fix. Deliberately NOT used. Recorded in
  klook.ts's comment only so this adid isn't regenerated by
  accident later.

Mechanical notes:
- KLOOK_HOTELS_WIDGET_SCRIPT_SRC renamed to KLOOK_WIDGET_SCRIPT_SRC -
  it was never actually hotels-specific (it's Klook's one generic
  `.klk-aff-widget` loader), and now visibly serves two different
  widgets on the same page.
- KlookTripSection.tsx now renders that loader script ONCE (a single
  `<Script id="klook-widgets-loader">`, placed after both widgets)
  instead of once per widget - the script scans the whole DOM for
  `.klk-aff-widget` elements in one pass, so a second copy would
  just load the same file twice for no benefit. Removed the second,
  duplicate `<Script>` tag that used to sit next to the hotels
  widget specifically.
- New widget reuses the `.klook-widget-mount` CSS class (the dark
  --ink frame, var(--border) outline) - this class went unused the
  moment the Tours Widget was deleted, so no new CSS was needed; the
  frame already matches what the user asked for ("black background
  same as viator grid" - see "Klook widget framing, corrected
  (2026-09-03)" above for why a literal dark iframe background isn't
  achievable and a frame is the honest alternative).
- This widget keeps data-cid/data-tid (unlike the hotels static
  widget above it) - it's Klook's destination+tag algorithm doing
  the picking here, not a hand-picked property list, so there's a
...
## Homepage redesign: matching the pasted mockup (2026-09-03)

The user pasted a full homepage screenshot with the instruction "this is how i want the homepage layout. same to same. exact design." Built to match it structurally and visually as closely as the real product supports: sticky top nav with section tabs, a two-column hero (copy left, illustration right) with a floating "Rate Verified" badge, a white search card overlapping the bottom of the hero band, a five-item trust-icon strip, an "Explore Dubai" section with two large cards, a "Top Hotels in Dubai" grid of four property cards with a solid-color image header and a "View rates" button, a "More than just hotels" Klook cross-sell band, then the existing "What makes Rate Manifest different" and footer sections unchanged.

This required breaking the homepage out of `.shell`, the single bordered/shadowed card that wraps every other page - the mockup's hero and trust strip run edge to edge, which a 960px bordered card can't do. `.home-page` has no max-width of its own now; `.home-content` restores a centered container (1160px, a bit wider than `.shell`'s 960px, matching the mockup's airier grid) for everything from "Explore Dubai" down. Every other page (`/search`, `/browse`, `/for-business`, etc.) still uses `.shell` untouched. `NavBar` got a `variant="home"` that drops the sticky-card-top styling (negative margins, bottom border, rounded top corners) in favor of a plain translucent bar that sits directly on the hero gradient - every other page still gets the default variant.

Three deliberate departures from the mockup, all on integrity grounds rather than taste, flagged to the user rather than silently built or silently dropped:

1. **No photograph of a specific real place for the hero.** The mockup used what reads as a real photograph of the Dubai skyline. No licensed photography was available (Artlist image generation returned `insufficient_credits`), and generating an AI image that *claims* to depict a specific real landmark and shipping it on a live commercial site carries its own misrepresentation risk independent of the credits issue. Built `HeroArt.tsx` instead - an abstract, brand-colored skyline illustration (inline SVG, themed off `--primary`/`--positive`) that reads as "Gulf coastline at golden hour" without claiming to be a photo of anywhere specific.

2. **No fabricated review/rating/trust data on hotel cards.** The mockup's cards show star ratings with specific review counts ("4.8 (12,456 reviews)"), and blanket "Verified"/breakfast/free-cancellation tags on every card. This app has no review data anywhere in its schema, and StayingAPI's own `price-compare` response doesn't return real cancellation terms (confirmed in `stayingApiRefresh.ts`, defaults to "not confirmed free" - see the earlier "Klook widget white cards" and "Cancellation Intelligence" entries for the same underlying limit). Homepage cards show only this app's own real `starRating` integer, and a genuinely computed "X% below comparable rates" badge - `browseCity()` (`src/lib/browse.ts`) now also returns `percentBelowAverage` (this hotel's cheapest cached offer vs. the average of its own other cached offers for the same dates, never a cross-hotel or invented "market rate," and never shown at all when there are fewer than two offers to average) and `hasFreeCancellationOffer` (true only when a cached offer's own `cancellations` row actually says so - realistically only ever true for mock hotels today, which is the honest current state). Both computed from data already cached, zero extra StayingAPI credits, same "cache-only reads" rule as `browseCity()` already followed.

3. **No competitor logos, and no "RATE MANIFEST VERDICT" / "Smart Insights" panel.** This site's own footer disclosure already states it is "not affiliated with or endorsed by Booking.com, Expedia, Agoda, Hotels.com, or Trip.com" - a "Trusted Partners" logo row naming Booking.com and Expedia in the same footer would directly contradict that, and the mockup's fifth logo, Tiqets, isn't integrated here at all (would be claiming a partnership that doesn't exist). The Verdict/Smart Insights block (a 92/100 score, a 14-day price trend chart, a per-night room-upgrade price, a location score) is exactly the Decision Intelligence feature set scoped separately the same day in `claude/decision-intelligence-roadmap.md` (a Project doc) - none of it has real data behind it yet (price history is sparse days after go-live, there's no geo/POI dataset, room-type depth per hotel isn't confirmed) - so it isn't shown with invented numbers on a live site real visitors see. Revisit once that roadmap's Phase 1/2 items actually exist.

Also deliberately unchanged from the mockup's apparent intent but scoped down to what's real: the search card keeps the existing Emirate/Property mode toggle and its real autocomplete rather than the mockup's four tabs (Hotels/Things To Do/Flights/Transfers) - this app has no flights or transfers data source at all, and Things To Do has never been a standalone searchable product here, only a per-hotel results-page addition (see KlookTripSection/ThingsToDoSection). The top nav's "Things To Do" tab links to the homepage's own new `#explore` section rather than a dead standalone route. No wishlist heart or account icon in the nav - no accounts system exists, and a nav icon that does nothing is worse than one that isn't there.

New files: `src/components/HeroArt.tsx` (skyline illustration), `src/components/TrustIcons.tsx` (five inline SVG icons for the trust strip). Verified with `npx tsc --noEmit` and `npm run build`, both clean, plus a local Playwright screenshot pass (Postgres + `next start`, real seeded catalog data) confirming the layout renders as intended - hero, floating search card, trust strip, Explore Dubai, and the real four-hotel grid all matched the mockup's structure with this app's own real data in place of the mockup's invented numbers.

## StayingAPI live-check failures - root cause confirmed: credits exhausted (2026-09-03)

The "We could not check real-time prices" error investigated earlier this session (hypothesized as likely credit exhaustion, never confirmed - both a direct call from this sandbox and a same-origin diagnostic call carrying the admin secret were correctly blocked) is now confirmed directly by the user checking the StayingAPI dashboard: the free-tier credit balance is at zero. Not a code bug, not a StayingAPI outage, not a mapping problem - the account is simply out of credits. The free 300-credit tier was down to ~120 two days before this note; it's since been spent to zero, most likely by the searches/refreshes run while diagnosing the widget/rebrand work earlier this session.

Practical effect right now: every real hotel's on-demand check (`ensureLiveCheckTriggered`) fails immediately and every real hotel's search results page shows the generic error state - this is the current live behavior for any real property, not an edge case. The GitHub Actions refresh workflow would fail the same way for the same reason, so it wasn't run once this was confirmed independently.

No code change follows from this - the failure handling (the `error` branch in search/page.tsx, the honest generic message rather than a fake price) was already correct. The fix is topping up or upgrading the StayingAPI plan, which is the user's call, not a build task - flagged in chat rather than assumed.

## Phase 1 (Layer A): Verified Rate panel, Is this a good price?, RateManifest Verdict (2026-09-03)

Approved in chat: "Yes - let me know if/when you want to do that, and whether you want me to build anything StayingAPI-dependent in the meantime knowing it'll show the honest 'not checked yet' state until credits are back. go ahead." This built the three Layer A gaps identified on `/search` (#1 Rate Verification, #3/#4 Is this a good price / Price History, #27 RateManifest Verdict), on purpose while StayingAPI credits are still at zero - the whole point is that these render an honest degraded state today and upgrade automatically to the fully verified state the moment credits are topped up, with no further code change.

Before this, a meaningful amount of Layer A already existed inside `ResultsList.tsx`: the per-offer deal-signal ring, the "Why this deal?" panel with honest factors (`bestDealScore.ts`, `dealSignal.ts`), the freshness badge, and best-offer-only badging. What was missing was making any of that visible without clicking into a single offer, and a price-history feature didn't exist at all yet.

**VerifiedRatePanel** (`src/components/VerifiedRatePanel.tsx`) - a dominant panel at the top of a real hotel's results, replacing the old small mono "N sources checked" line as the primary trust signal. Three states, matching the three real outcomes `ensureLiveCheckTriggered` can leave a hotel/date pair in:
- `verified` - a cache row exists with at least one available offer. Green accent, checkmark icon, "Checked X ago - N sources compared - availability confirmed."
- `no-availability` - a cache row exists (the check really happened) but nothing came back available. Neutral, not alarming - this is a real, checked answer.
- `not-checked` - no cache row and the live attempt just failed. This is the state every real hotel shows right now, since StayingAPI credits are at zero (see the entry above). Deliberately amber, not red, and deliberately not phrased as an error about the property - "We couldn't run a real-time check for these exact dates just now. This isn't about this property."

Mock hotels render none of this - the existing `.demo-banner` already says the prices are simulated, and a "Verified" badge next to simulated data would contradict it in the same breath.

**"Is this a good price?"** (`src/lib/priceInsight.ts` + `src/components/PriceInsightPanel.tsx`) - the first thing this app has ever read back out of `price_history`, the durable ledger `runSearch()` has been writing to since it existed. Collapses every non-sold-out row for one hotel/check-in date to one cheapest-total-per-calendar-day, then reports a lowest/highest/average range across those days. The honesty rule carried over from `bestDealScore.ts`'s reliability component: below `MIN_OBSERVATION_DAYS_FOR_PRICE_INSIGHT` (3) distinct days of observation, this shows "we don't have enough observations yet," never a range built from one or two points dressed up as a trend. Most properties will show this sparse-data branch at launch - price history only accumulates from real searches against the exact check-in date being viewed, so that's the honest starting state, not a bug.

**RateManifestVerdict** (`src/components/RateManifestVerdict.tsx`) - the per-offer score/tier/verdict, promoted to a page-level summary a visitor sees without opening the "Why this deal?" toggle. Deliberately reads the same top-ranked offer and the same `getDealSignal()`/factor list every other verdict on the page already uses (the per-offer factor list was pulled out of `ResultsList.tsx` into `src/lib/scoring/factors.ts` so both places build it from one function, not two copies that could drift) - two different "our recommendation" numbers on one page would be its own kind of dishonesty.

All three verified locally against the real (non-mock) 30-hotel catalog: a manually seeded "ready" StayingAPI cache row plus four price_history rows across distinct days rendered the fully verified state with a real AED 855-980 range and a "2% above average" verdict; a seeded "ready" row with no price history rendered the correct sparse-data message ("observed this property 1 time so far... need at least 3"); an untouched real hotel with no cache row rendered the honest "Not checked yet" panel - which is exactly what the live site shows today for any real hotel, confirmed against `oberoi-dubai` and `address-downtown` with no seed data at all. All three states passed `npx tsc --noEmit` and `npm run build` before delivery. Seed rows were removed from the local test database after the screenshots.

## The Analyse This Hotel gate, and the full Intelligence journey (2026-09-04)

Prompted by two things in the same conversation. First, the question "why is it still having staying api as primary search" - the real answer: landing on any hotel (from the homepage's Top Hotels grid, `/browse`, or SearchForm) went straight to `/search`, which calls `ensureLiveCheckTriggered()` on page load. Browsing a hotel and spending a StayingAPI credit on it were the same action - there was no moment where a visitor was just looking. Second, a detailed corrected product spec pasted in chat, giving the exact canonical Intelligence journey and copy: Your Hotel, Rate Verified, Is This A Good Price, Why This Deal, Where To Book, RateManifest Verdict, Before You Book, Recheck, Book - explicitly ranked ahead of Phase 2/3 (price trend, alternative dates, room upgrades, the "what am I really getting" offer-comparison feature), all of which stay deferred.

**New route: `/hotel`** (`src/app/hotel/page.tsx`) - the free browse step. Shows the hotel, dates, occupancy, room type, and whatever's already cached (via a new `browseHotel()` in `src/lib/browse.ts`, the same zero-credit read `browseCity()` already does, factored out to a single hotel) with one link onward: "Analyse This Hotel" to `/search`, unchanged. `/search` itself is untouched in terms of when it spends a credit - `ensureLiveCheckTriggered()` still runs exactly where it always did. What changed is that nothing links to `/search` directly anymore except that one explicit button - the homepage's Top Hotels grid, `/browse`'s hotel grid, and SearchForm's submit target all now point at `/hotel` first.

**`/search` restructured into the canonical order.** New components: `YourHotelSummary` (step 1, shown on both `/hotel` and `/search` so a direct/bookmarked `/search` link still has context - hotel, dates, nights, occupancy, room type, all real fields off the `rooms` table, deliberately with no invented "selected rate" line, since this app compares several offers rather than pre-selecting one), `WhyThisDealPanel` (step 4, a plain checklist of only the positive factors `buildDealFactors()` actually knows, plus "rate was successfully verified" and a historical-average line when `priceInsight` supports it), and `BeforeYouBookPanel` (steps 8-9, a checklist built entirely from fields already on the page - hotel, dates, guests, room, cancellation, total price, rate-verified timestamp - nothing new fetched). `VerifiedRatePanel`'s verified state now states the actual price and night count ("AED 940 for 1 night"), not just a source count. `PriceInsightPanel` gained a plain-language standing label ("Strong price" / "Good price" / "Typical price" / "Above the average we've seen") derived from the exact same `percentVsAverage` number already in the paragraph beneath it - not a second, invented score. `RateManifestVerdict` moved from just under Rate Verified to the very end, right before Before You Book, and its old inline factor list moved out into `WhyThisDealPanel` so the same checklist isn't shown twice.

**"Recheck Before Booking" is deliberately NOT a real re-check.** There is no cache-invalidation or force-refresh mechanism anywhere in the StayingAPI integration - `ensureLiveCheckTriggered()` only ever fires a live call when no cache row exists yet for a hotel/date pair; once one exists, every later visit reads the same cached row, however old, forever. Building a button that forces a brand-new paid StayingAPI call on demand is a real, separate feature with its own cost and abuse tradeoffs (would every visitor get to trigger a paid call at will, with credits this scarce?) - not something to quietly invent as a side effect of this pass. So `BeforeYouBookPanel`'s "Reload for the latest check" link does exactly what it says - reloads the page, showing whatever's already on file - and says so honestly rather than implying a fresh verification that isn't happening. Flagged in chat as a decision that needs the user's input if a real force-recheck feature is wanted later.

**Also addressed directly in chat, no code change:** the StayingAPI root-cause finding (credits at zero) was independently confirmed via the user's own dashboard check, not a guess - already recorded above, not re-diagnosed. The Klook -> StayingAPI property-matching/confidence layer (Gate 2) stays blocked on Gate 1 (neither Klook nor KKday has confirmed hotel-search API capability yet) - nothing to build there until a discovery supplier confirms.

Verified locally: `/hotel` renders the free gate correctly (cached preview badges, "Analyse This Hotel" link) for a seeded hotel; the full restructured `/search` renders all eight sections in order for the same seeded hotel, including a real "Typical price" standing label from six accumulated observation days; the honest "Not checked yet" real-hotel state still renders correctly with the new Your Hotel/gate structure in place. `npx tsc --noEmit` and `npm run build` both clean. Seed rows removed from the local test database after screenshots.

## Klook browsing moved to the top of the homepage (2026-09-04)

Prompted by chat, 2026-09-04, with a screenshot of the live homepage: "this needs to check thru klook or any supplier. why is rate manifest first????? ... let the customer search broadly: Dubai -> dates -> guests -> hotels/properties. They see the Klook hotel inventory and can browse as many properties as they want. No expensive StayingAPI intelligence needs to run here. i cant give you smaller than this instruction."

The literal ask (an embedded search box, inside RateManifest, where a visitor types dates and guests and Klook's whole Dubai hotel inventory searches in place) is not a capability Klook offers, and this needed saying plainly rather than quietly built around. Two independent confirmations, not a guess:
- Gate 1, established earlier in this project: Klook's public partner API is OCTO-based - tours/activities/attractions only, no hotel-search endpoint at all.
- Re-read `src/lib/klook.ts` and `KlookTripSection.tsx` directly for this change: `KLOOK_HOTELS_WIDGET_CONFIG` is Klook's affiliate *widget* program, not search - a fixed panel of four hand-picked 5-star Dubai properties ("By property" mode in Klook's dashboard), rendered by Klook's own script inside an iframe this app doesn't control, with no dates/guests inputs and no way to add any. It is advertising, not a search UI.

So "browse Klook's inventory, searched by your own dates and guests" is real, but only on klook.com itself. The honest build: make Klook's real content the first thing on the homepage, with a real widget for a first taste and a prominent outbound link to Klook's own Dubai hotel search (`KLOOK_HOTELS_LINK`) for anyone who wants to actually search by dates and guests - not a fabricated in-page search experience.

**New component: `KlookHomeBrowse`** (`src/components/KlookHomeBrowse.tsx`) - mounted first inside the hero band, above RateManifest's own picker. Reuses the exact same real widget (`KLOOK_HOTELS_WIDGET_CONFIG`) already live on `/search`'s secondary "also on Klook" block, plus a large CTA button to `KLOOK_HOTELS_LINK` ("Search all Klook hotels in Dubai - pick your dates & guests"), with disclosure copy that says plainly that the four properties shown are a fixed set and that date/guest search only happens once a visitor follows the link to Klook's own site. Zero StayingAPI cost - this component never touches `runSearch()` or `stayingApiRefresh.ts`.

**RateManifest's own picker (`SearchForm`, inside `.home-search-card`) demoted, not removed.** It was never a broad "browse the market" tool to begin with - it's a lookup ("Find my rate ->") for a property you already have in mind, unchanged in that respect. Moved below the new Klook panel and given a plain heading ("Already know the hotel? Look it up here for RateManifest's verified rate.") so its job is stated rather than implied by being first. Same visual weight as before (white card, shadow) - the fix here is ordering and framing, not shrinking a tool that still works.

**Also fixed while in this file, a pre-existing mislabel, not part of the request but directly adjacent:** the homepage's "Things To Do"/"Iconic Experiences" card and the "Discover tours, activities, and experiences" band both linked to `KLOOK_HOTELS_LINK` (Klook's hotels search) under experiences/tours copy. Both now point at `KLOOK_LINK` (Klook's general Dubai search-results page), matching what `KlookTripSection.tsx` on `/search` already does correctly and what the copy on both cards actually promises.

**Also renamed the picker's "Emirate" mode to "Emirate/City"** - separate small ask in the same conversation ("make it emirate/city, choose a property (lets keep it open for anyone wants to plan out of uae)"). The picker itself was already generic (it lists whatever distinct `city` values exist in the `hotels` table, nothing UAE-specific in the logic), but the label implied UAE-only. Updated the tab label, the select's aria-label ("Emirate or city"), and its default option ("All emirates/cities") in `SearchForm.tsx` - no logic change, since none was needed for this to already work with a non-UAE city the moment one is added to the catalog.

Verified locally: `npx tsc --noEmit` and `npm run build` both clean; screenshot of the rebuilt homepage confirms the Klook panel renders first (eyebrow, real widget mount, CTA, disclosure) directly under the hero, with the RateManifest picker's new "Already know the hotel?" heading visible immediately below it. The widget iframe itself doesn't render inside this sandbox - `affiliate.klook.com` isn't reachable from here, same limitation already noted for the `/search` widgets - so the fallback "Klook.com" link is what the screenshot shows in its place; this is a dev-environment network limitation, not a rendering bug, and matches how the existing `/search` widgets have always had to be verified.

## Klook links refreshed; SearchForm simplified to a single Property lookup; Top Hotels made city-aware (2026-09-04)

Four small, related requests in the same chat as the Klook-homepage change above, addressed together since they all touch the same homepage/SearchForm surface.

**Klook links refreshed.** `KLOOK_LINK` and `KLOOK_HOTELS_LINK` (`src/lib/klook.ts`) both replaced with links the user generated directly and pasted in chat - "Link for - Explore Activities- https://affiliate.klook.com/sl/1DjS3Cm" and "Link for View Hotels- https://affiliate.klook.com/sl/1yrhErq." These come from Klook's own affiliate short-link tool rather than the Travelpayouts links used before - matched to the two constants by what each one is actually for (Explore Activities is the experiences CTA, KLOOK_LINK; View Hotels is the hotels CTA, KLOOK_HOTELS_LINK), not by trusting the button copy alone. Also dropped "in Dubai" from KlookHomeBrowse's CTA per instruction - "Search all Klook hotels - pick your dates & guests" now, not "...in Dubai...".

**SearchForm simplified to a single Property lookup.** Chat: "under 'already know the hotel' - just keep one tab for 'Property' with a memory selection below it as he types. (we dont need emirates or choose a property, etc, as he is coming to rate manifest after selecting the hotel to verify rate.)" `SearchForm.tsx` rewritten: the Emirate/City + Property mode toggle is gone, along with the Emirate `<select>` pair and its "Browse all hotels" fallback link - what's left is just the type-ahead property search (unchanged logic) with a plain "Property" label. Removed the now-dead `.mode-toggle`/`.mode-tab`/`.emirate-picker`/`.browse-all-link` CSS rather than leave unused rules behind. `/browse` (linked from the Top Hotels section) is still the place to browse by location - this card was never that tool once KlookHomeBrowse took over as the homepage's primary browsing surface earlier the same day, so trimming it to just the lookup it's actually for is consistent with that change, not a new direction.

**Top Hotels made city-aware.** Chat: "'Top Hotels in Dubai' should only say 'Top Hotels'. The shortlist should auto reflect the city he is searching property in... we can have a city tab to select in the beginning, that force feeds that city and the site only reflects options for that city." `HomePage` (`src/app/page.tsx`) now reads a `?city=` query param, validated against the real distinct `city` values in the `hotels` table (defaulting to Dubai when present, else the first city alphabetically, and falling back the same way on an invalid/unknown value) - `browseCity()` runs against that city instead of a hardcoded `"Dubai"`. A row of plain link-based city pills (`.home-city-tabs`, real navigation to `/?city=<name>`, not client state) sits above the Top Hotels grid; picking one re-renders the whole page server-side against that city, same pattern `/hotel` and `/search` already use for their own query params. "Top Hotels in Dubai" (both the section heading and the matching card inside "Explore Dubai") is now just "Top Hotels", and that card's link now follows the selected city too.

Deliberately NOT made city-aware: the "Explore Dubai" section's own heading and its "View all →" link (still `/browse?city=Dubai`) - left as the flagship-market overview it already was, not asked to change. The Klook sections (KlookHomeBrowse, the "Discover tours..." band) stay Dubai-labeled on purpose - Klook's fixed hotels widget genuinely only shows Dubai properties today, so a city-aware label there would misrepresent what's actually behind it, not just reorganize real data like the Top Hotels change does.

Verified locally: `npx tsc --noEmit` and `npm run build` both clean; screenshots of the rebuilt homepage at the default (Dubai, 4 real Dubai properties, city pill active) and at `/?city=Sharjah` (4 real Sharjah properties - 72 Hotel Sharjah, Corniche Hotel Sharjah, Pullman Sharjah, Sheraton Sharjah Beach Resort & Spa - Sharjah pill active, "View all hotels" pointed at `/browse?city=Sharjah`) both confirm the shortlist and its links follow the selected city correctly.

## Klook's real Partner API, read directly - hotels stay blocked, activities/tours booking looks genuinely possible (2026-09-04)

Prompted by chat: the user pasted a generic "custom code vs CMS/widget" explainer about Klook integration and asked directly whether RateManifest can take Klook bookings fully on its own site via API, with no affiliate redirect. Rather than answer from the earlier Gate 1 writeup alone, fetched Klook's own current public API docs directly (klook.gitbook.io/openapi) and its partner page (klook.com/partner/) to check this against the real thing, not a summary of it.

**Hotels: still no, reconfirmed a third time, now against Klook's own docs directly.** The API's documented product categories are explicitly listed - Attractions, Tours & Activities, Theme Parks & Water Parks, Food & Beverage, Wi-Fi & SIM Cards, Transportation Passes. No hotels/accommodation category anywhere in it. This is the same conclusion Gate 1 already reached (OCTO-scoped, tours/activities only), now confirmed by reading the current docs directly instead of relying on the earlier research pass. Nothing changes for the hotels work already shipped this session (KlookHomeBrowse, the widget + outbound-search-link architecture) - it remains the only honest option for hotels.

**Activities/tours: a real, previously-unexplored opening.** The OpenAPI spec file backing Klook's docs is literally named `OCTO-API-OCTO-API-1.0-resolved.json` - confirms OCTO, but OCTO is not catalog-only. The docs show a genuine transactional booking flow: an availability check, then `POST /bookings` to create a reservation, `POST /bookings/{uuid}/confirm`, `POST /bookings/{uuid}/cancel`, plus `GET /bookings`. That's a real create/confirm/cancel booking lifecycle, not just a product list - which means, on paper, RateManifest could complete a Klook tour/activity booking on its own site without redirecting to klook.com, for the Things To Do category KlookTripSection's affiliate widgets already touch today.

**What the public docs don't say, and would need a direct answer from Klook before any of this gets built:** whether this "Distributor" API tier is even open to a site RateManifest's size (klook.com/partner/ splits Distributor/Supplier/affiliate as separate programs - this is not the same signup as the existing Travelpayouts-mediated affiliate account already in use); how payment and settlement actually work (does RateManifest collect the customer's payment and become the seller of record, or does checkout still run through Klook even though the booking is created via API - materially different in cost, liability, and what "on our site" actually means); and what support/refund/liability obligations RateManifest would be taking on as the apparent seller of a tour it doesn't operate, which today sits entirely with Klook via the affiliate redirect. None of this is answerable from public documentation - it needs a real application/conversation with Klook's partnership team, the same way the KKday and Klook-hotels enquiries already in flight do.

No code written and none should be yet - this is a feasibility finding, not a decision to integrate. Flagged in chat and in the project's decision-intelligence-roadmap doc as a live option distinct from the still-blocked hotels question; next step if the user wants to pursue it is drafting an application/enquiry to Klook's Distributor/API program, same pattern as the existing KKday outreach draft.
